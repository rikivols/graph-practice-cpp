...
public:
    ...
    void populate_gifts_table(const vector<pair<Price, Gift>> &sorted_prices) noexcept {
        ...
        for (size_t i: employees_topsorted) {
            ...
            // we only need to iterate on our employee till the max second-cheapest gift (+1), because
            // that's the max gift that the employee's underlink can get
            Gift max_gift = 0;

            for (Employee underling: employees[i].underlings) {
                if (cheapest_employee_gifts[underling].id > max_gift) {
                    max_gift = cheapest_employee_gifts[underling].id;
                }
                if (second_cheapest_employee_gifts[underling].id > max_gift) {
                    max_gift = second_cheapest_employee_gifts[underling].id;
                }
            }

            size_t max_size = min(employees[i].underlings.size() + 2, sorted_prices.size());
            size_t iterate_gifts_till = min(max_gift + 3, max_size);

            // column of the table, representing the gift
            for (size_t j=0; j<iterate_gifts_till; j++) {
...
void speed_test1() {
    ...
    Price expected = 3000000;

    auto start = chrono::high_resolution_clock::now();
    test(expected, boss, gifts);
