...
public:
    ...
    T extract_min() {
        ...
        T minElement = std::move(elements[0].value);
    ...
    void bubbleUp(size_t child_index) {
        while (true) {
            ...
            // if son < parent, swap
            if (comp(elements[child_index].value, elements[parent].value)) {
    ...
    void bubbleDown(size_t parentIndex) {
        while (true) {
            ...
            int smallest_son = get_smallest_son(parentIndex);
            ...
            if (comp(elements[smallest_son].value, elements[parentIndex].value)) {
                swap(elements[smallest_son], elements[parentIndex]);
                parentIndex = smallest_son;
            }
            else {
                break;
            }
    ...
    Ref push(T val) {
        elements.push_back(Node(std::move(val)));
        Ref ref(elements[elements.size()-1]);
    ...
    int get_smallest_son(size_t parent_index) {
        int leftSon = (1 + (int)parent_index) * 2 - 1;
        int rightSon = leftSon + 1;

        if (leftSon >= (int)elements.size()) {
            return -1;
        }

        int smallest_son = leftSon;
        if (rightSon < (int)elements.size()) {
            if (comp(elements[rightSon].value, elements[leftSon].value)) {
                smallest_son = rightSon;
            }
        }
    ...
    void fix_heap_value(size_t index) {
        size_t parent = (index-1) / 2;

        if (index != 0 and comp(elements[index].value, elements[parent].value)) {
            bubbleUp(index);
            return;
        }

        int smallest_son = get_smallest_son(index);

        if (smallest_son != -1 and comp(elements[smallest_son].value, elements[index].value)) {
            bubbleDown(index);
        }
    }
    ...
    void change(const Ref& ref, Fun f) {
        ...
        f(elements[index].value);

        fix_heap_value(index);
    }
    ...
    T erase(const Ref& ref) {
        ...
        T erased_element = std::move(elements[index].value);

        swap(elements[index], elements[elements.size()-1]);
        elements.pop_back();
