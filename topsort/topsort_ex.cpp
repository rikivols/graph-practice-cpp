...
//  std::cout << "INPUT GRAPH: " << G;
  ...
  while (!nextVertex.empty()) {
      Vertex curVertex = nextVertex.front();
      nextVertex.pop();

      topologicalSort.push_back(curVertex);

      for (auto neighbourVertex: G[curVertex]) {
          outputCounter[neighbourVertex]--;
          if (outputCounter[neighbourVertex] == 0) {
              nextVertex.push(neighbourVertex);
          }
      }
  }

//  std::cout << "output counter: ";
//  printArr(outputCounter, vertexNum);
  ...
  if (isLoop) {
      std::vector<Vertex> loop = retractLoop(G, outputCounter);
//      std::cout << "LOOP: ";
//      printVector(loop);
      return {false, loop};
  }
